...
def callback():
    ...
    if 'access_token' in json:
        ...
        return render_template('layouts/error.html', title='MLH Server Error',
                               message="We're having trouble pulling your information from MLH servers. This is a fatal error. Please contact {} for assistance".format(
                                   settings.GENERAL_INFO_EMAIL)), 505
...
def confirm_registration():
    ...
    if request.method == 'GET':
        if current_user.status != 'NEW':
            ...
            return render_template('layouts/error.html', title='Confirm Account',
                                   message='Please check your email to confirm your account before proceeding'), 403
        return render_template('users/confirm.html', user=current_user)
    else:
        ...
        try:
            if int(num_hackathons) > 9223372036854775807:
                flash(
                    "{} seems like a lot of hackathons! I don't think you've been to that many".format(num_hackathons),
                    'error')
